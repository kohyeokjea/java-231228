...
// 상속해주는 클래스(Parent): 조상 클래스, 부모 클래스, 상위 클래스, 기반 클래스, 슈퍼 클래스
// 상속받는 클래스(Child): 자손 클래스, 자식 클래스, 하위 클래스, 파생 클래스, 서브 클래스
...
// !상속 예시2
// : Tv 클래스로부터 상속받고 기능을 추가하여 SmartTv클래스를 작성
class Tv {
	// 전원 상태(on/off)
	boolean power;
	// 채널
...
class SmartTv extends Tv {
	...
	void displayOtt(String platform) {
		// OTT의 상태가 on(true)일 때만 platform을 보여줌
		if (ott) {
